...
```{r word_freq_all, echo=FALSE, warning=FALSE, message=FALSE,fig.width=16, fig.height=9}
bncs_mot_comm_fil_old_1 <- bncs_mot_comm_fil_old %>%
  select(-freq_cpwd) %>%
  arrange(desc(freq)) %>%
  mutate(rank = 1:nrow(bncs_mot_comm_fil_old)) %>%
  gather(dataset, rel_freq, c(rel_freq_mot, rel_freq_bnc_sub, rel_freq_bnc)) %>%
  group_by(dataset) %>%
  mutate(rel_freq_cum = cumsum(rel_freq)) 

bncs_mot_comm_fil_old_2 <- bncs_mot_comm_fil_old %>%
  select(-freq) %>%
...
plot1 <- ggplot(bncs_mot_comm_fil_old_1, aes(x=log(rank), y=rel_freq_cum, fill=dataset))+
  geom_point(colour="grey")+
  geom_smooth(size=1, colour="black", aes(linetype = dataset)) +
  ...
  scale_linetype_manual("Dataset", labels = c("BNC-all","BNC-match","Mothers"),values=c("dotted", "longdash", "solid"))+
  guides(fill=FALSE, alpha=F)

plot2 <- ggplot(bncs_mot_comm_fil_old_2, aes(x=log(rank), y=rel_freq_cum, fill=dataset))+
  geom_point(colour="grey")+
  geom_smooth(size=1, colour="black", aes(linetype = dataset)) +
  scale_y_continuous(labels=scales::percent, limits = c(0,1)) +
  ggtitle("Children word printed database")+
  theme(axis.text=element_text(size=18),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        plot.title = element_text(size=20),
...
grid_arrange_shared_legend(plot1, plot2, position = "right", 
                           x_title = "Word rank (log scale)", y_title = "Cumulative proportion of tokens",
                           x_size = 25, y_size = 25,
...
bncs_mot_comm_fil_1 <- bncs_mot_comm_fil %>%
  select(-freq_cpwd) %>%
  arrange(desc(freq)) %>%
  mutate(rank = 1:nrow(bncs_mot_comm_fil)) %>%
  gather(dataset, rel_freq, c(rel_freq_mot, rel_freq_bnc_sub, rel_freq_bnc)) %>%
  group_by(dataset) %>%
  mutate(rel_freq_cum = cumsum(rel_freq)) 

bncs_mot_comm_fil_2 <- bncs_mot_comm_fil %>%
  select(-freq) %>%
  arrange(desc(freq_cpwd)) %>%
  mutate(rank = 1:nrow(bncs_mot_comm_fil)) %>%
  gather(dataset, rel_freq, c(rel_freq_mot, rel_freq_bnc_sub, rel_freq_bnc)) %>%
...
plot1 <- ggplot(bncs_mot_comm_fil_1, aes(x=log(rank), y=rel_freq_cum, fill=dataset))+
  geom_point(colour="grey")+
  geom_smooth(size=1, colour="black", aes(linetype = dataset)) +
  scale_y_continuous(labels=scales::percent, limits = c(0,1)) +
  ggtitle("Spoken BNC")+
...
plot2 <- ggplot(bncs_mot_comm_fil_2, aes(x=log(rank), y=rel_freq_cum, fill=dataset))+
  geom_point(colour="grey")+
  geom_smooth(size=1, colour="black", aes(linetype = dataset)) +
  scale_y_continuous(labels=scales::percent, limits = c(0,1)) +
  ggtitle("Children word printed database")+
...
```{r word_freq_raw, echo=FALSE, warning=FALSE, message=FALSE,fig.width=16, fig.height=9}
...
plot1 <- ggplot(bncs_mot_comm_raw_fil_1, aes(x=log(rank), y=rel_freq_cum, fill=dataset))+
  geom_point(colour="grey")+
  geom_smooth(size=1, colour="black", aes(linetype = dataset)) +
  scale_y_continuous(labels=scales::percent, limits = c(0,1)) +
  ggtitle("Spoken BNC")+
...
plot2 <- ggplot(bncs_mot_comm_raw_fil_2, aes(x=log(rank), y=rel_freq_cum, fill=dataset))+
  geom_point(colour="grey")+
  geom_smooth(size=1, colour="black", aes(linetype = dataset)) +
  scale_y_continuous(labels=scales::percent, limits = c(0,1)) +
  ggtitle("Children word printed database")+
